...
{
	connect(socket, &RcTcpSocket::readyToTransfer, this, &RateController::scheduleTransfer);
	socket->setReadBufferSize(m_downLimit * 4);
	m_sockets << socket;
	scheduleTransfer();
}
...
{
	disconnect(socket, &RcTcpSocket::readyToTransfer, this, &RateController::scheduleTransfer);
...
void RateController::setDownloadLimit(int bytesPerSecond)
{
	...
	for (RcTcpSocket *socket : m_sockets)
		socket->setReadBufferSize(m_downLimit * 4);
}
...
void RateController::transfer()
{
	...
	for (RcTcpSocket *client : m_sockets) {
	...
	do {
		...
		while (it.hasNext() && (bytesToWrite > 0 || bytesToRead > 0)) {
			...
			qint64 available = qMin<qint64>(socket->networkBytesAvailable(), readChunk);
			if (available > 0) {
			...
			if (m_upLimit * 2 > socket->bytesToWrite()) {
				...
				qint64 toWrite = qMin(m_upLimit * 2 - socket->bytesToWrite(), chunkSize);
				if (toWrite > 0) {
